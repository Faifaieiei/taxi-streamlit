...
st.set_page_config(
    page_title="DBSCAN Clustering (Mon)",
    page_icon="📁",
    layout="wide"
)
...
taxi_data = pd.read_csv('D:\Senior Project\OD_2022\Monday_bkk.csv')
# แสดงข้อมูล DataFrame ให้แน่ใจว่าโหลดข้อมูลได้อย่างถูกต้อง
# st.write(taxi_data.head())
# print(f"{taxi_data.shape=}")
...
st.title("CLUSTER BY DBSCAN ON MONDAY")
...
with tab1:
    ...
    with st.form("my form"):
        ...
        session_state.num_start = st.selectbox("ENTER STARTTIME", list(range(25)), format_func=lambda x: str(x) if x != 0 else "Choose start time")
        session_state.num_end = st.selectbox("ENTER ENDTIME", list(range(1, 25)), format_func=lambda x: str(x) if x != 0 else "Choose end time")
        ...
        if submitted:
            ...
            filtered_mon = taxi_data[(taxi_data['starttime'].dt.hour >= session_state.num_start) & (taxi_data['starttime'].dt.hour < session_state.num_end)]
            ...
            st.write(taxi_subset.head())
            session_state.taxi_subset = taxi_subset
            ...
            st.write(session_state.points) # save points 
...
with tab2:
    ...
    with st.form("form"):
        ...
        session_state.range_start = st.number_input("ENTER START RANGE")
        session_state.range_end = st.number_input("ENTER END RANGE")

        submitted = st.form_submit_button("view results")
        
        # กำหนดช่วงของ eps และ minPts
        eps_range = np.arange(session_state.range_start, session_state.range_end, 1)
        ...
        if submitted:
            ...
            best_eps = None
            best_minPts = minpts(session_state.points)
            ...
            for eps in eps_range:
                ...
                dbscan.fit(session_state.points)
                    
                # คำนวณ silhouette score
                labels = dbscan.labels_
                silhouette_avg = -1 if len(set(labels)) <= 1 else silhouette_score(session_state.points, labels)
            ...
            # input parameter
            db = DBSCAN(eps=best_eps, min_samples=best_minPts).fit(session_state.points)
            ...
            # คำนวณ Silhouette Score
            silhouette_avg = silhouette_score(session_state.points, dblabels)
            ...
            filtered_mon_in_selected_district = session_state.taxi_subset
